// src/app/[locale]/page.tsx - Updated with i18n support
"use client";
...
export default function AdvisorComparison({ params }: AdvisorComparisonProps) {
  ...
  const getDetailedAnalysis = (advisor: AdvisorData) => {
    ...
    if (lowScoreItems.length > 0) {
      if (lowScoreItems.length <= 3) {
        risks.push(t('analysis.deficiencies', { items: lowScoreItems.join(t('analysis.separator')) }));
      } else {
    ...
    if (totalScore >= 80) {
      ...
      suggestion = lowScoreItems.length > 0 
        ? t('analysis.someRisks', { items: lowScoreItems.slice(0, 2).join(t('analysis.and')) })
        : t('analysis.generalRisks');
    } else {
      suggestion = lowScoreItems.length > 0
        ? t('analysis.majorRisks', { items: lowScoreItems.slice(0, 3).join(t('analysis.separator')) })
